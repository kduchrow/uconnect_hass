...
class Uconnect_API: 
    ...
    def auth_response(self) -> str:
        ...
        r = session.get(f'{self._loginUrl}/accounts.login', params=parameter , cookies=self._cookies)
    ...
    def get_jwt_token(self) -> str:
        parameter = self._login_default_params_dict.copy()
        parameter['fields'] = 'profile.firstName,profile.lastName,profile.email,country,locale,data.disclaimerCodeGSDP'
        parameter['login_token'] = self._login_token
        
        session = requests.session()
        
        r = session.get(f'{self._loginUrl}/accounts.getJWT', params=parameter , cookies=self._cookies)
    ...
    def get_aws_credentials(self) -> str:
        ...
        IdentityId=self._identity_id,
            Logins={
                'cognito-identity.amazonaws.com':  self._identity_token
            },
        )
    ...
    def get_aws_data(self, endpoint: str) -> json:
        ...
        auth = AWSRequestsAuth(aws_access_key=self._aws_access_key,
    ...
    def post_aws_pin_authenticate(self, pin: str) -> json:   
        ...
        endpoint = f'{self._authUrl}/v1/accounts/{self._user_id}/ignite/pin/authenticate'
